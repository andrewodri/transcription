...
var HtmlTransformer = (function () {
  ...
  _prototypeProperties(HtmlTransformer, {
    transform: {
      value: function transform(files, template) {
        var htmlFiles = new Map();

        var navTemplate = jade.compileFile(template + "/index.jade");
        var classTemplate = jade.compileFile(template + "/class.jade");

        var navigation = navTemplate({ files: Array.from(files.values()) });
          ...
          for (var _iterator = files[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            ...
            try {
              for (var _iterator2 = file.classes[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                ...
                htmlFiles.set(classObject.name.toLowerCase() + ".html", classTemplate({ data: classObject, navigation: navigation }));
              }
            } catch (err) {
